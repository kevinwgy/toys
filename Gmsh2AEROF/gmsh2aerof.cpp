...
//               MSH format to AERO-F "TOP" files.
//        Usage: <path to the MSH file (input)> <path to the TOP file (output)>
...
static map<int,pair<string,int> > groups; //Maps GMSH-MSH physical group to AERO-F volume & boundary type, and number of elems
...
{
  elemType[2] = pair<int,int>(3,4); //3-node triangle
  elemType[3] = pair<int,int>(4,2); //4-node quadrangle
  elemType[4] = pair<int,int>(4,5); volTypes.insert(4); //4-node tetrahedron
//  elemType[5] = pair<int,int>(8,6); volTypes.insert(5); //8-node hexahedron

  groups[0] = pair<string,int> (string("FluidMesh_0"),0);  //added - for the volume mesh
  groups[1] = pair<string,int> (string("OutletFixedSurface_1"),0);
  groups[2] = pair<string,int> (string("SymmetrySurface_2"),0);
  groups[3] = pair<string,int> (string("StickFixedSurface_3"),0);
  groups[4] = pair<string,int> (string("InletFixedSurface_4"),0);

  if(argc!=3) {
    fprintf(stderr,"input format: [binary] <path to MSH file (input)> <path to TOP file (output)>\n");
    exit(-1);
  }
  ...
  //cout << "NOTE: For AERO-F input, the mesh files need to be combined into a single file." << endl;
  string command = "cat " + outputfilename + ".surf* >> " + outputfilename;
  const char *command_str = command.c_str();
  system(command_str);
  string command2 = "rm " + outputfilename + ".surf*";
...
void ProcessMSH4(ifstream &input, ofstream &output, string outputfilename)
{
  ...
  for(int i=0; i<nSurfaces; i++) {
    ...
    if(nPhyTag > 1) 
      cout << "WARNING: Detected multiple (" << nPhyTag << ") physical tags for surface entity " << id << ".\n";
    for(int j=0; j<nPhyTag; j++) {
      input >> phyTag;
      entity2surface.insert(pair<int,int>(id, phyTag));
      if(groups.find(phyTag) == groups.end()) {
        cout << "Found unrecognizable physical tag: " << phyTag << ". Aborting.\n";
        exit(-1);
      }
      ...
      if(it == surfaceTopo.end()) {
        ...
        (*surfaceTopo[phyTag]) << "Elements " << groups[phyTag].first << " using FluidNodes\n";
      }
    }
    getline(input,text);
  }
  ...
  output << "Nodes FluidNodes\n";

  int entityDim, entityTag, parametric, nNodesInBlock;
 // cout << "nBlocks = " << nBlocks << endl;
  for(int iBlock=0; iBlock<nBlocks; iBlock++) {
    //cout << "working on block " << iBlock+1 << endl;
    input >> entityDim >> entityTag >> parametric >> nNodesInBlock;
    //cout << "working on block " << iBlock+1 << ". nNodesInBlock = " << nNodesInBlock << "." << endl;
  ...
  int volumeCounter = 0;
  if (nVolumes > 0) 
    output << "Elements " << groups[0].first << " using FluidNodes\n";
  ...
  for(int iBlock=0; iBlock<nBlocks; iBlock++) {
    ...
    if (entityDim == 2) { //found a 2d entity
      ...
      int aerofElemTag = elemType[currentElemType].second;
      ...
      for (int i=0; i<nElemsInBlock; i++) {
        ...
        for(auto it=ret.first; it!=ret.second; it++) {
          phyTag = it->second;
          (*surfaceTopo[phyTag]) << std::setw(10) << ++surfaceTopoCounter[phyTag] << std::setw(6) << aerofElemTag;
        }
  ...
  if(volumeTopo.size()==1) 
    cout << "Number of 3D elements: " << volumeCounter << "." << endl;
  else if(volumeTopo.size()>1) {
  ...
  for(auto it = surfaceTopoCounter.begin(); it != surfaceTopoCounter.end(); it++)
    cout << "Number of 2D elements in " << groups[it->first].first << ": " << it->second << endl;
...
void ProcessMSH2(ifstream &input, ofstream &output)
{
  ...
  //  cout << ind << " " << type << " " << nTag << " " << group << " " << tag2 << endl;
    ...
    if (group != currentGroup) {// write title line
      ...
      output << "Elements " << groups[group].first << " using FluidNodes" << "\n";
      cout << "Writing " << groups[group].first << " to file." << endl;
      if (groups[group].second != 0) 
        cout << "WARNING: The same group '" << groups[group].first << "' has appeared before. You may need to manually merge different segments." << endl;
    }

    if (elemType.find(type) == elemType.end()) {
      cout << "ERROR: Unknown/unsupported element type " << type << " for element " 
           << ind << ". Aborting." << endl;
      exit(-1); 
    }
