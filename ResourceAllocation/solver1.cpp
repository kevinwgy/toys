...
int main(int argc, char *argv[])
{
  ...
  MPI_Comm comm;
  MPI_Comm_split(global_comm, SOLVER1_COMM_TAG+1, global_rank, &comm);

  int rank(0), size(0);
  MPI_Comm_rank(comm, &rank);
  MPI_Comm_size(comm, &size);

  fprintf(stdout,"I am proc %d (of %d) --- global %d (of %d): running solver 1.\n",
          rank, size, global_rank, global_size);

  int maxcolor = 10;
  int color = SOLVER1_COMM_TAG;
  ...
  for(int i=0; i<maxcolor; i++) {
    if(i != color && newleaders[i] >= 0) {
      ...
      nSolvers = i+1; //tracks the number of colors/solvers
    }
  }
